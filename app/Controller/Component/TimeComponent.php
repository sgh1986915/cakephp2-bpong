...
/**
 * Time Component class for easy use of time data.
 * Import to /app by Edward
...
class TimeComponent extends Component
{
    ...
    /**
 *  converting meridian foramt to 24 format 24 -00
 * @param $hour
 * @param $minutes
 * @param $meridian
 * @return array (['hour'],['min'])
 */
    function AmPmTo24($hour=null,$minutes=null,$meridian=null) 
    {
        if ($meridian && $meridian=="pm" && $hour != 12) {            
            $hour = $hour + 12;
        }  
        
        if ($hour == 24 || ($hour == 12 && $meridian == 'am')) {
            $hour = '00';
        }
        
        return array('hour'=>$hour,'min'=>$minutes);        
    }        
    
    /**
     *  Converting time to sql format  16:12:07
     *  @author vovich
     */
    function timeToSql($hour=null,$minutes=null,$meridian=null) 
    {
        $sql = "";
        if ($meridian && $meridian=="pm" && $hour != 12) {            
            $hour = $hour + 12;
    ...
    /**
* 
 * Converts given date string from d/m/Y format to Y-m-d
 * @author Edward
 * @param string $str time string to convert
 * @return string converted one
 */
    function calendarToSql($str) 
    {
 ...
 * @param  mixed $options  Default format string, if timestamp is used in $dateTime, or an array of options to be passed on to timeAgoInWords().
