...
require_once('class.FedexTags.php');
...
class FedExDC extends FedExTags{
        ...
        */
		...
		function TrackNumberFEDEX($tracking_number){
			$track_Ret = $this->track(
		    	array(
		        	'1537' => $tracking_number, //Tracking Number
			        '1534' =>'Y' // detail_scan_indicator (Show me all the tracking data)
		    	)
			);
			$result_array = $this->ParseFedExReply();
			return $result_array;
		}

		function ParseFedExReply(){
			$result_array = array();
			$hash = $this->rHash;
			if ($error = $this->getError()) {
				...
				$result_array['packages_count'] = $hash['1584'];
		    	for ($i=1; $i<=$hash['1584']; $i++) {
		    		if(isset($hash['1720-'.$i])){
			    		if ($hash['1720-'.$i] != '') {
			    			$result_array['packages'][$i]['status'] = 'delivered';
			    			$result_array['packages'][$i]['delivery_datetime'] = $this->format_datetime($hash['1720-'.$i], $hash['1707-'.$i]);
			    			$result_array['packages'][$i]['signed_by'] = $hash['1706-'.$i];
			    		}
		    		}
		    		$summarize = '';
		    		for ($num = 1; $num<$hash['1715-'.$i]; $num++){
		    			if(isset($hash['1171-'.$i.'-'.$num])){
		    			if ($hash['1171-'.$i.'-'.$num] == '') {
		    				// additional exception info in 1168 and 1711
		    				$scan_code = '';
		    			} else {
		    				$scan_code = $hash['1171-'.$i.'-'.$num];
		    			}
		    			}
		    			$result_array['packages'][$i]['history'][] = array(
		    														'scan_code' => $scan_code,
		    														'description' => $hash['1159-'.$i.'-'.$num],
		    														'location_city' => $hash['1160-'.$i.'-'.$num],
		    														'location_state' => $hash['1161-'.$i.'-'.$num],
		    														'datetime' => $this->format_datetime($hash['1162-'.$i.'-'.$num], $hash['1163-'.$i.'-'.$num]),
		    														'location_country' => $hash['1164-'.$i.'-'.$num],
		    			 											);
		    		}
		    		$records=array();
		    		if(isset($result_array['packages'][$i]['history'])){
		   				$records = $result_array['packages'][$i]['history'];
		    		}
		   			$result_array['packages'][$i]['summary']='';
		    		for ($num = 0; $num<$hash['1715-'.$i]-1; $num++){
		    			$result_array['packages'][$i]['summary'] .= '<b>'.$records[$num]['datetime'].': </b>'.$records[$num]['location_city'].', '.$records[$num]['location_state'].', '.$records[$num]['location_country'].'.<br><i><font color=blue>'.$records[$num]['description'].'</font></i><br>';
		    		};
