...
$this->preg_autolinks = array(
    'pattern' => array(
        '#(https?|ftp|news){1}://([\w\-]+\.([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^"\s\(\)<\[]*)?)#ie',
        '#([\s\(\)])(www|ftp)\.(([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^"\s\(\)<\[]*)?)#ie',
        "#(^|[\n ])([a-z0-9&\-_.]+?)@([\w\-]+\.([\w\-\.]+\.)*[\w]+)#i"
    ),
    'replacement' => array(
        '\' \'.smarty_modifier_linkify(\'$1://$2\')',
        '\'$1\'.smarty_modifier_linkify(\'$2.$3\', \'$2.$3\')',
...
//$path = str_replace('\\', '/', $path) . 'img/bbcode/smiles/';
//$pak = file($this->_current_path . 'images/smiles/Set_Smiles_YarNET.pak');

$path = 'http://'.$_SERVER['HTTP_HOST'].'/img/bbcode/smiles/';
$pak = file(WWW_ROOT . 'img/bbcode/smiles/Set_Smiles_YarNET.pak');
