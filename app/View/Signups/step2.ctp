...
function showResponseStep2(responseText)  {
  if (responseText=="Error") {
  		$('#error-message').show('slow');
  		$('#next').hide();
  		alert("Error while updating");
  } else {
		$.get("/signups/checkAddress/", function(data){
			if(data!=''){
				$('#next').html("<input type='submit'  value='Next' class='submit'  />");
				alert (data);
			}else{
				<?php if (!empty($packages) ):?>
					<?php if ($isFreeEvent):?>
  					window.location.href = '<?php echo SECURE_SERVER?>/signups/saveFreeSignup/<?php echo $modelName."/".$slug; ?>';
					<?php else:?>
					    $.ajaxSetup({cache:false});
	  					$('#SignupAjax').load("/signups/step3/<?php echo $modelName."/".$slug ?>",{cache: false},function(){});
	  				<?php endif;?>
  				<?php else:?>
  					window.location.href = '<?php echo MAIN_SERVER;?>';
  				<?php endif;?>
...
function Previos() {
    ...
    $('#previos').html(loader);
    $.ajaxSetup({cache:false});
	$('#SignupAjax').load("/signups/step1/<?php echo $modelName."/".$slug ?>",{cache: false});
}

</script>

<?php echo $this->Form->create('User',array('id'=>'Step2','name'=>'Step2','url'=>'/signups/step2/'.$modelName.'/'.$slug));?>
   ...
   <div class="left35"><h4>Home address information</h4></div>
	...
	<div class='clear'><br/></div>
  	 ...
  	 <div style="padding:15px 0 0 20px">
  	 	<div class='step_previous' id="previos"  style="width:100px; float:left"><input type="button"  value="Previous" class="submit" onclick="Previos();" /></div>
        <div id="next" class='step_next' style="width:100px; float:left"><input type="submit"  value="Next" class="submit"  style='margin-top:0px;'/></div>
    </div>
</form>