...
?>
<?php $this->pageTitle = 'All ' . $this->Formater->authorsName($user['User'], 1, $isAuthor) . ' Links' ?>
<h2 class='hr'>All <?php echo $this->Formater->authorsName($user['User'], 1, $isAuthor); ?> Links</h2>
<table class='sub_list sorter' cellpadding="0" cellspacing="0">
    ...
    foreach ($items as $item):
    	...
    	if (isset ($item[$model]['title'])) {	
    	    $item[$model]['name'] = $item[$model]['title'];   	        
    	} else {
    	    $item[$model]['name'] = '';     	    
    	}   	
    ?>
    	...
    	<tr<?php echo $class;?>>
			<td class='center'><?php echo $prevNums + $j;?></td>	   	
                <?php echo $this->element('/submissions/element_link', array('item' => $item[$model]));	?>  	
			<td>
