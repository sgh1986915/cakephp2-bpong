<?php if (!empty($edit)):?>
	<?php echo $this->Form->create('Event',array('enctype'=>"multipart/form-data",'id'=>'EventEditForm','name'=>'EventEditForm','action'=>'edit'));?>
<?php 
	    echo $this->Form->hidden('Venue.model', array('value' => 'Event'));
	    echo $this->Form->hidden('Venue.model_id', array('value' => $this->request->data['Event']['id']));
?>
...
<div id="create_venue"  style="display: none;">
  ...
  <?php
				echo $this->Form->input ('Venue.name', array('label' => 'Name <span class="red">*</span>', 'value' => ''));
				echo $this->Form->input ('Venue.venuetype_id', array('value' => ''));
				echo $this->Form->hidden ("Venue.id",  array('id'=>'venueId'));
...
?>
  ...
  <!-- <div id="map_canvas1" style="width: 500px; height: 500px;"></div> -->
    <?php    
				 ...
				 echo $this->Form->input ( 'Venue.Address.country_id', array ('label' => 'Country <span class="red">*</span>', 'type'=> 'select', 'id' => 'venue_country_id','options' => $countries, 'value' => ''));//, array('onchange' => 'requestAddressChange();') 
				 echo $this->Form->input ( 'Venue.Address.provincestate_id', array ('type'=> 'select','options' => $states, 'label' => 'State or Province', 'id' => 'venue_state', 'value' => ''));//, array('onchange' => 'requestAddressChange();')
				 echo $this->Form->input ( 'Venue.Address.address',array('label' => 'Address <span class="red">*</span>', 'id'=>'street', 'value' => ''));//, array('onchange' => 'requestAddressChange();')
				 echo $this->Form->input ( 'Venue.Address.city',array('label' => 'City <span class="red">*</span>', 'id'=>'city', 'value' => ''));//, array('onchange' => 'requestAddressChange();') 
				 echo $this->Form->input ( 'Venue.Address.postalcode',array('id'=>'zip', 'label' => 'Postal code', 'value' => '') );
				 echo $this->Form->input ( 'Venue.web_address', array('value' => ''));
				 echo $this->Form->input ( 'Venue.Phone.phone', array('value' => ''));
				 echo $this->Form->input ( 'Venue.Phone.type', array ('type'=> 'select','options' => $phonetype, 'value' => ''));
				 echo $this->Form->input ( 'Venue.description', array('value' => ''));
		?>
...
<?php if (!empty($edit)):?>
	...
	<?php if (!empty($this->request->data['Venue'])) :?>
		<br class='clear' />
		...
		<tr>
			<td> <a target="blank" href="/venues/view/<?php echo $this->request->data['Venue']['slug']; ?>"> <?php echo $this->request->data['Venue']['name']; ?></a></td>
			<td><?php echo $this->request->data['Venue']['phone']; ?></td>
			<td><?php echo @$this->request->data['Venue']['Address']['address']; ?></td>
			<td><?php echo @$this->request->data['Venue']['Address']['city']; ?></td>
			<td><?php echo @$this->request->data['Venue']['Address']['state_name']; ?></td>
		</tr>
	</table>
