...
<?php $markersCount = count($markers); $satelliteMarkersCount = count($satelliteMarkers);?>
    var venueLocations = [
    <?php
    $i = 0;
...
var satelliteLocations = [
    <?php
    $i = 0;
    foreach ($satelliteMarkers AS $satelliteMarker) :
    $i++;                                                                     
    ?>
        <?php if (!empty($satelliteMarker['EventView']['id']) && !empty($satelliteMarker['Venue']['longitude'])) : ?>
                [
            '<div class="balloon-content">\n\
            <strong><a href="/event/<?php echo $satelliteMarker['EventView']['id'];?>/<?php echo addslashes($satelliteMarker['EventView']['slug']);?>"><?php echo addslashes($satelliteMarker['EventView']['name']);?>
            </a></strong>'+
            '<br/>'+
            '<span class="element-header">Start date: </span>'+
            '<span><?php echo $this->Time->niceShort($satelliteMarker['EventView']['start_date']); ?></span>'+
            ''
            <?php if (strstr($satelliteMarker['EventView']['end_date'],'0000-00-00') === false): ?>
            +'<br/>'+
                '<span class="element-header">End date: </span>'+
                '<span><?php echo $this->Time->niceShort($satelliteMarker['EventView']['end_date']); ?></span>'+
            ''
            <?php endif; ?>
            +'<br/>'+'<span class="element-header">Venue: </span>'+
            '<span><a href="/venues/view/<?php echo $satelliteMarker['Venue']['slug']; ?>"><?php echo ucwords(strtolower(addslashes($satelliteMarker['Venue']['name']))); ?></a></span>'+
            '<br/>'+
            '<span class="element-header">Venue address: </span>'+
            '<span><?php echo ucwords(strtolower(addslashes($satelliteMarker['Venue']['address']))); ?></span>'+
            ', <span><?php echo ucwords(strtolower($satelliteMarker['Venue']['city'])); ?></span>'
            <?php if (isset($satelliteMarker['Venue']['shortname'])): ?>
                +' <span><?php echo ucwords(strtolower(addslashes($satelliteMarker['Venue']['shortname']))); ?></span>'
            <?php endif; ?>
            +'</div>',
            <?php echo $satelliteMarker['Venue']['latitude']; ?>,
            <?php echo $satelliteMarker['Venue']['longitude']; ?>,
            <?php if ($satelliteMarker['Venue']['nbpltype'] == 'None')
                    echo false;
                  else
                    echo true; ?>
        ]
    <?php if ($i != $satelliteMarkersCount):?>
        ,
    <?php endif; ?>
        <?php endif; ?>
    <?php endforeach; ?>
    ...
    function initializeMap() {
        ...
        for (var i = 0; i < satelliteLocations.length; i++) {
            var location = satelliteLocations[i];
            var point = new google.maps.LatLng( location[1], location[2] );
            createSatelliteMarker(map, point, location[0], infowindow,location[3]);
        }
    ...
    function createVenueMarker(map, point, infoContent, infowindow) {
        var image = new google.maps.MarkerImage("<?php echo IMG_NBPL_LAYOUTS_URL;?>/marker_venue.png",
            // This marker is 20 pixels wide by 32 pixels tall.
            new google.maps.Size(19, 18),
    ...
    function createSatelliteMarker(map, point, infoContent, infowindow,isNBPLVenue) {
        
        var image;
        image = new google.maps.MarkerImage("<?php echo IMG_NBPL_LAYOUTS_URL;?>/marker_satellite.png",
                // This marker is 20 pixels wide by 32 pixels tall.
                new google.maps.Size(19, 18),
                // The origin for this image is 0,0.
                new google.maps.Point(0,0),
