...
?>
<?php $this->pageTitle = 'All ' . $this->Formater->authorsName($user['User'], 1, $isAuthor) . ' Images' ?>
<h2 class='hr'>All <?php echo $this->Formater->authorsName($user['User'], 1, $isAuthor); ?> Images</h2>
<?php if (empty($items)):?>
No Images <br/><br/>
<?php else:?>
    ...
    ?>
    	...
    	<tr<?php echo $class;?>>
			<td class='center'><?php echo $prevNums + $j;?></td>	   	
                <?php echo $this->element('/submissions/element_image', array('item' => $item[$model]));	?>  	
			<td>
