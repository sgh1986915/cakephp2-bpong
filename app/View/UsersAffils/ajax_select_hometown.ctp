...
function saveAffil () {
	...
	if (!affilName) {
		alert('Please specify Hometown Name');
		return false;
	}
	if (!affilCountryID) {
	...
	$.ajaxSetup({cache:false});
	$.post("/users_affils/save_city/Hometown", { 'name': affilName, 'country_id': affilCountryID, 'state_id': affilStateID},
			   function(data) {
...
<h1 class="login"><img src="<?php echo STATIC_BPONG;?>/img/logclose.png" id="Close" class="right"  onclick="self.parent.tb_remove();" />Select Your Hometown</h1>
