...
<div class="blogposts">
        ...
        <?php echo $this->Form->create('Blogpost', array('enctype'=>"multipart/form-data",'url' => array('action' => 'edit'), 'id' => 'Addpost'));?>
            <fieldset style="padding-left:0px;">
            <?php
                echo $this->Form->hidden('Blogpost.id');
                echo $this->Form->input('Blogpost.title', array('size' => 70));
            ?>
        <div class="textarea">
                <?php
                echo $this->Form->input('Blogpost.description', array('rows' => 20, 'div' => false));
                echo $this->Image->manageImages('Blogpost', false, $images, 10, array('default_alt' => $this->request->data['Blogpost']['title']));  
            ?>
        </div>
