<?php $this->pageTitle = 'Beer Pong Video Album - ' . $album['Album']['name']; ?>
<?php
...
<h2 class='hr'>
    	...
    	case 'User': ?>
			<?php echo $this->Formater->authorsName($album['User'], 1, $myProfile); ?> Video Album
    		<?php
    	break;
    	case 'Event': ?>
				Event Video Album
    ...
    <?php endif;?>
    <?php if ($canUpload):?>
     <span class='hr_subtext'>|</span> <?php echo $this->Html->link('Add More Videos', '/submit/video/album/' . $album['Album']['id']);?>
    <?php endif;?>
</h2>
<div style='text-align:center;width:100%;margin-bottom:7px;'>
    ...
    <div>
        <b><?php echo $total;?></b> <?php echo $this->Language->pluralize($total, 'video', 'videos'); ?> in album     <?php echo $this->Html->link($album['Album']['name'], '/Albums/show_video/' . $album['Album']['id']);?>
    </div>
</div>
...
<div class='album_image'>
		<a href="/video/<?php echo $video['Video']['id'];?>" style='background-image:url("<?php if ($video['Video']['id']) { ?><?php echo $this->Youtube->getVideoImage($video['Video']['youtube_id']);?><?php } else { ?><?php echo STATIC_BPONG?>/img/emptyVideo.jpg<?php }?>");'>  </a>
</div>
<?php if ($i == 4): $i=0;?>
<br class='clear'/>
<?php endif;?>
...
<?php } else {?>
<div style='width:100%;padding:20px;text-align:center;'><strong>No Videos</strong></div>
<?php }?>
