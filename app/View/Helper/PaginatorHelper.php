 ...
 */
	function sortKey($model = null, $options = array()) {
		...
		// changed by Oleg D.			
		elseif (!empty($this->options['url']['sort'])) {
		    return $this->options['url']['sort'];
		}		
 ...
 */
	function sortDir($model = null, $options = array()) {
		...
		if (isset($options['direction'])) {
	     ...
	     // for custom query pagination 
		// changed by Oleg D.			
		elseif (!empty($this->options['url']['direction'])) {
		    $dir = $this->options['url']['direction'];
		}					
		elseif (isset($options['order']) && is_array($options['order'])) {
 ...
 */
	function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			if($params['page'] >= 4 && $params['page'] <= 8) { $start = 4; }
			if($end >= ($params['pageCount'] - 3) && $params['page'] <= ($params['pageCount'] - 3)) { $end = $params['pageCount'] - 3; }
 ...
 */
	function first($first = '<< first', $options = array()) {
		...
		extract($options);
		unset($options['after'], $options['model'], $options['separator'], $options['modulus']);

		$out = '';

		if (is_int($first) && $params['page'] > $first && $params['pageCount'] > $modulus) {
			if ($after === null) {
				$after = '.&nbsp;.&nbsp;.';
			}
			for ($i = 1; $i <= $first; $i++) {
			...
			$out .= $after;
		} elseif ($params['page'] > $first && $params['pageCount'] > $modulus) {
 ...
 */
	function last($last = 'last >>', $options = array()) {
		...
		extract($options);
		unset($options['before'], $options['model'], $options['separator'], $options['modulus']);

		$out = '';
		$lower = $params['pageCount'] - $last + 1;

		if (is_int($last) && $params['page'] < $lower && $params['pageCount'] > $modulus) {
			if ($before === null) {
				$before = '.&nbsp;.&nbsp;.';
			}
			...
			$out = $before . $out;
		} elseif ($params['page'] < ($params['pageCount']-$last) && $params['pageCount'] > $modulus) {
			$out = $before . '<span>' . $this->link($last, array('page' => $params['pageCount']), $options) . '</span>';
		}
		return $out;
	}
	/**
	 * Change sorting TD of sorted model
	 * @author Oleg D.
	 */
	function showActiveClass ($key) {
        if ($this->sortKey() == $key) {
            return ' sorted_td';    
        } else {
            return false;
