<?php
/* SVN FILE: $Id: time.php 6643 2011-10-03 18:31:16Z _skinny $ */
 ...
 * @lastmodified	$Date: 2011-10-03 21:31:16 +0300 (Пн, 03 окт 2011) $
 ...
 */
	function fromString($dateString, $userOffset = null) {
		/*Achtung added by vovich for working with the User timezone*/
//		if  (isset($_SESSION['loggedUser']) && !empty($_SESSION['loggedUser']['timezone'])){
//			$userOffset = $_SESSION['loggedUser']['timezone'];
//		}
 ...
 * @return string Formatted date string
 */
	function niceForEvent($dateString = null, $userOffset = null) {
		if ($dateString != null) {
			$date = $this->fromString($dateString, $userOffset);
		} else {
			$date = time();
 ...
 */
	...
	function niceDateJS($dateString = null, $userOffset = null) {
		if ($dateString != null) {
			$date = $this->fromString($dateString, $userOffset);
		} else {
			$date = time();
 ...
 */
	function niceShort($dateString = null, $userOffset = null) {
		...
		if ($this->isToday($date)) {
			$ret = "Today, " . date("h:i a", $date);
		} elseif ($this->wasYesterday($date)) {
			$ret = "Yesterday, " . date("h:i a", $date);
		} else {
			$ret = date("M jS{$y}, h:i a", $date);
		}
