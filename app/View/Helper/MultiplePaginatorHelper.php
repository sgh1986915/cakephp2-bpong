<?php
/* SVN FILE: $Id: multiplepaginator.php 5378 2011-05-02 14:27:01Z odikusar $ */
/**
 ...
 * @lastmodified	$Date: 2011-05-02 17:27:01 +0300 (Пн, 02 май 2011) $
 ...
 * @subpackage	cake.cake.libs.view.helpers
 */
class MultiplePaginatorHelper extends AppHelper {
 ...
 */
	function options($options = array()) {
		...
		if (!empty($options['paging'])) {
			...
			$this->request->params['paging'] = am($this->request->params['paging'], $options['paging']);
		...
		if (!empty($options[$model])) {
			...
			$this->request->params['paging'][$model] = am($this->request->params['paging'][$model], $options[$model]);
			unset($options[$model]);
		}
 ...
 */
	function sort($title, $key = null, $options = array()) {
		...
		$url = am(array('sort' => $key, 'directions' => $dir), $url, array('order' => null));
		return $this->link($title. ($dir == 'asc' ? '' : '') , $url, $options);
	}
/**
 ...
 */
	...
	//echo "<pre>d";
		...
		if(isset($options['global_url'])){
			$global_url=$options['global_url'].'/page:'.$url['page'];
		}else{
			$global_url=Set::filter($url, true);
		};

		return $this->{$obj}->link($title, $global_url, $options);
	}
/**
 ...
 */
	function __pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		$my_model=$options['model'];
		if(!isset($options['model'])) $my_model=NULL;
		$this->{$check}($my_model);
		$options = am($_defaults, $options);
		$paging = $this->params($options['model']);
        $options['model'];
		if (!$this->{$check}($my_model) && (!empty($disabledTitle) || !empty($disabledOptions))) {
			if (!empty($disabledTitle) && $disabledTitle !== true) {
				$title = $disabledTitle;
			}
			$options = am($options, $disabledOptions);
		} elseif (!$this->{$check}($my_model)) {
			return null;
		}
 ...
 */
	function counter($options = array()) {
		...
		$paging = $this->params($options['model']);
		$paging['pageCount'] = !empty($paging['pageCount'] == 0) ? 1 : $paging['pageCount'];

		$start = !empty($paging['count'] >= 1) ? ($paging['page'] - 1) * ($paging['options']['limit']) + 1 : '0';
		$end = !empty(($paging['count'] < ($start + $paging['options']['limit'] - 1))) ? $paging['count'] : ($start + $paging['options']['limit'] - 1);
